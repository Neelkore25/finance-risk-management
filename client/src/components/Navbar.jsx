    ...
    <header className="sticky top-0 z-30 h-16 bg-white dark:bg-[#080F1A] border-b border-slate-200 dark:border-white/10 px-4 lg:px-8 flex items-center justify-between shadow-sm dark:shadow-lg transition-colors duration-150">
      ...
      <div className="flex items-center gap-4 flex-1 max-w-md" ref={searchRef}>
        <button
          ...
          className="p-2 rounded-xl text-slate-600 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-white dark:hover:bg-[#162335] lg:hidden border border-slate-200 dark:border-white/10"
        >
        ...
        <div className="relative w-full hidden sm:block">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-1.5 bg-slate-100 dark:bg-[#0D1724] border border-slate-200 dark:border-white/10 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors"
          />
          ...
          {isSearchOpen && searchQuery.trim() !== '' && (
            <div className="opaque-dropdown w-full left-0 mt-2 max-h-80 overflow-y-auto p-2 space-y-1 z-50">
              <div className="px-2 py-1 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider border-b border-slate-200 dark:border-white/10">
              ...
              {filteredRoutes.length > 0 ? (
                filteredRoutes.map((route) => (
                  <button
                    ...
                    className="w-full text-left p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-[#162335] transition-colors flex items-center justify-between group"
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[9px] px-1.5 py-0.2 rounded bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 uppercase font-semibold">
                          {route.category}
                        </span>
                      </div>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate max-w-xs mt-0.5">{route.desc}</p>
                    </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          <button
            ...
            className="p-2 rounded-xl bg-slate-100 dark:bg-[#0D1724] text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-white/10 transition-colors relative"
            title="System Notifications"
          >
            <Bell className="w-4 h-4" />
            {alerts.length > 0 && (
              <span className="w-2 h-2 rounded-full bg-rose-500 absolute top-1.5 right-1.5 ring-2 ring-white dark:ring-[#080F1A] animate-pulse" />
            )}
          </button>

          {showNotifs && (
            <div className="opaque-dropdown w-80 right-0 mt-2 p-3 space-y-2.5 z-50">
              <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-white/10 text-xs font-bold text-slate-900 dark:text-white">
                <span>Active Risk Alerts</span>
                <span className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full ${
                  alerts.length > 0 ? 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-400' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400'
                }`}>
              ...
              {alerts.length > 0 ? (
                    ...
                    <div
                      ...
                      className={`p-2.5 rounded-xl border ${
                        alert.severity === 'Critical'
                          ...
                          ? 'bg-amber-50 border-amber-200 text-amber-800 dark:bg-amber-950/40 dark:border-amber-800/80 dark:text-amber-200'
                          : 'bg-blue-50 border-blue-200 text-blue-800 dark:bg-blue-950/40 dark:border-blue-800/80 dark:text-blue-200'
                      }`}
                    >
                      <div className="flex items-center gap-2 mb-1">
                        <AlertTriangle className="w-3.5 h-3.5 shrink-0" />
                        <p className="font-bold text-slate-900 dark:text-white text-xs">{alert.title}</p>
                      </div>
                ...
                <div className="py-4 text-center space-y-1.5">
                  ...
                  <p className="text-xs font-bold text-slate-900 dark:text-white">All Parameters Stable</p>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400">No critical threshold breaches detected.</p>
                </div>
              )}
            </div>
          )}
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-[#0D1724] border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:hover:border-slate-600 transition-colors shadow-sm"
          >
            ...
            <div className="text-left hidden md:block">
              ...
              <span className="text-[10px] font-semibold text-blue-600 dark:text-blue-400 block uppercase leading-none">
                {roleDisplay}
              </span>
            </div>
          </button>
          ...
          {dropdownOpen && (
            <div className="opaque-dropdown">
              <div className="p-3 border-b border-slate-200 dark:border-white/10">
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">{user?.email || 'admin@analytics.com'}</p>
                <span className="inline-block mt-1 px-2 py-0.5 rounded bg-blue-50 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 text-[10px] font-bold uppercase">
                  Role: {roleDisplay}
                </span>
              </div>
              <div className="py-1">
