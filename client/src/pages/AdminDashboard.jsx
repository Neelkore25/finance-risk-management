...
export function AdminDashboard() {
  ...
  if (userProfile?.role !== 'admin') {
    return (
      <div className="max-w-2xl mx-auto my-12 p-8 bg-white dark:bg-slate-900 border border-rose-200 dark:border-rose-800 rounded-3xl text-center space-y-4 shadow-xl">
        <div className="w-16 h-16 rounded-full bg-rose-50 dark:bg-rose-950/80 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-800 flex items-center justify-center mx-auto">
        ...
        <div className="p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-[11px] text-slate-500 dark:text-slate-400">
    ...
    <div className="space-y-6">
      ...
      <div className="opaque-card flex items-center justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <ShieldAlert className="w-6 h-6 text-sky-500 dark:text-sky-400" />
            <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">Admin Operational Dashboard</h1>
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Privacy-Preserving Aggregate Platform Metrics (SECURITY DEFINER Database Guard)
          </p>
        </div>
        <span className="px-3 py-1 bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300 text-xs font-bold rounded-full border border-emerald-200 dark:border-emerald-800">
          Role: Admin Verified
        </span>
      </div>

      {error && (
        <div className="p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 dark:bg-rose-950 dark:border-rose-800 dark:text-rose-300 text-xs">
          {error}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="opaque-card space-y-1">
          ...
          <span className="text-2xl font-black text-slate-900 dark:text-white">{metrics?.total_users || 0}</span>
        ...
        <div className="opaque-card space-y-1">
          ...
          <span className="text-2xl font-black text-slate-900 dark:text-white">{metrics?.total_financial_profiles || 0}</span>
        ...
        <div className="opaque-card space-y-1">
          ...
          <span className="text-2xl font-black text-slate-900 dark:text-white">{metrics?.avg_dti_ratio || 0}%</span>
        ...
        <div className="opaque-card space-y-1">
          ...
          <span className="text-2xl font-black text-slate-900 dark:text-white">{metrics?.avg_savings_rate || 0}%</span>
          <span className="text-[10px] text-slate-500 block">Anonymized Savings Metric</span>
        </div>
      </div>

      <div className="opaque-card space-y-3">
        <h3 className="text-sm font-bold text-slate-900 dark:text-white">System Security & Privacy Compliance</h3>
